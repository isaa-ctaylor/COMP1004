...
function displayData() {
    ...
    // Clear existing rows
    incomeTable.querySelectorAll('tr:not(.hidden):not(.add-row)').forEach(row => row.remove());
    expensesTable.querySelectorAll('tr:not(.hidden):not(.add-row)').forEach(row => row.remove());

    data.income.forEach(row => {
        rowElement = insertTableRow(incomeTable);
        rowElement.querySelector('[data-name="description"]').children[0].children[0].value = row.description;
        rowElement.querySelector('[data-name="frequency"]').children[0].children[0].value = row.frequency;
        rowElement.querySelector('[data-name="amount"]').children[0].children[0].value = row.amount;
        rowElement.querySelector('[data-name="date"]').children[0].children[0].value = row.date;
    });

    data.expenses.forEach(row => {
        rowElement = insertTableRow(expensesTable);
        rowElement.querySelector('[data-name="description"]').children[0].children[0].value = row.description;        
        rowElement.querySelector('[data-name="frequency"]').children[0].children[0].value = row.frequency;
        rowElement.querySelector('[data-name="amount"]').children[0].children[0].value = row.amount;
        rowElement.querySelector('[data-name="date"]').children[0].children[0].value = row.date;

...
function validateTables() {
    ...
    tables.forEach(table => {
        tableRows = table.querySelectorAll('tbody tr:not(.hidden):not(.add-row)');
        
        tableRows.forEach(row => {
            fields = ['description', 'frequency', 'amount', 'date'];

            fields.forEach(field => {
                cell = row.querySelector(`[data-name="${field}"]`);
                value = getFieldValue(cell);
                toggleError(cell, value === '');
                if (value === '') {
                    allCellsGood = false;
                }
            });
...
function doDataDownload() {
    allCellsGood = validateTables();
    console.log(allCellsGood);
    if (!allCellsGood) {
...
function onTableUpdate() {
    ...
    expensesRows = expensesTableBody.querySelectorAll('tr:not(.hidden):not(.add-row)');
    // Create data object
    data = {
        'income': [],
        'expenses': []
